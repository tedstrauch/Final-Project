...
# looking at the adjusted R-squared measure, we can see that 47% of the variation in child mortality rates in 2015 can be explained by the GDP/economy. The other 53% is due to error or other vaiables not accounted for in this model.
...
# Scenario 3
# The question is: Does the child mortality rate impact the happiness score? 
# Test Assumptions
# Testing for Linearity by looking at a scatterplot.

scatter.smooth(x=Happy2015.2017$Child_Mortality_2015TUK, y=Happy2015.2017$Happiness.Score.2015, main="Child Mortality Rate impact on Happiness Score 2015")
lmMod <- lm(Happiness.Score.2015~Child_Mortality_2015TUK, data=Happy2015.2017)
par(mfrow=c(2,2))
plot(lmMod)
lmtest::bptest(lmMod)
car::ncvTest(lmMod)
...
# All assumptions were met here too. 
# with this we can conclude that the slope is not zero. There is sufficient evidence to lead us to believe Child Mortality Rate somehow influences the happiness score in all the countries in our data in 2015. 
# here we have a t test that is significant at p < .001 meaning Child Mortality Rate is a significant predictor of Happiness score.
# Our adjusted R-squared tells us that 28% of the happiness score can be explained by the Child Mortality Rate. The other 72% is due to error or other variables not accounted for in this model.
